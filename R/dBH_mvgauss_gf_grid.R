...
dBH_mvgauss_gf_grid <- function(zvals, Sigma,
    ...
    if (res_init$safe){
        ...
        if (if_thresh_expt){
            ## Threshold out hypotheses with cond_exp below 
    ...
    cand_info <- sapply(cand, function(i){        
        ...
        grid <- gridfun(min(low, abs(zvals[i])), high, gridsize, side)
        rejfun <- lapply(1:ntails, function(k){
            knots <- abs(grid[[k]])
            extraknot <- abs(zvals[i])
            if (!(extraknot %in% knots)){
                knots <- c(knots, extraknot)
            }
            knots <- sort(knots)
            nrejs <- sapply(2:length(knots), function(j){
                tmp <- recover_stats_mvgauss(zvals[i], knots[j] * (-1)^(k-1), s, cor)
                zvals_tmp <- rep(0, n)
                zvals_tmp[i] <- tmp[1]
                zvals_tmp[-i] <- tmp[-1]
                ...
                pvals_tmp <- zvals_pvals(zvals, side)
        ...
        if (res_init$safe){
            expt <- sapply(rejfun, function(fun){
                ...
                sum((pnorm(fun$knots[inds + 1]) - pnorm(fun$knots[inds])) / fun$nrejs[inds])
            })
            expt0 <- NA            
            expt <- sum(expt) * n
            ifrej <- expt <= alpha
