...
dBH_mvt_gf <- function(zvals, Sigma,
                       sigmahat, df,
    ...
    low <- qt(alpha0 * max(avals) / n / ntails, df = df, lower.tail = FALSE)
    ...
    cand_info <- sapply(obj$cand, function(i){
        ...
        if (is_safe){
            expt <- sapply(res, function(re){
                thr <- pmin(tstat, re$thr)
                compute_cond_exp(tstat, re$knots, re$nrejs, thr, dist = function(n){pt(n, df = df)})
            })
            expt0 <- NA            
            expt <- sum(expt) * n
            ifrej <- expt <= alpha
            ...
            expt <- sapply(res, function(re){
                compute_cond_exp(tstat, re$knots, re$nrejs, re$thr, dist = function(n){pt(n, df = df)})
            })
            expt0 <- sum(expt) * n            
            if (expt0 <= alpha){
                expt <- sapply(res, function(re){
