...
load("../data/dBH_mvgauss_aggregate.RData")
...
for (i in 1:nrow(settings)){
    data <- res %>%
        ...
        mutate(safe = factor(safe,
                             levels = c("FALSE", "TRUE"),
                             labels = c("BH-type methods", "BY-type methods")))
    ...
    model_tmp <- switch(settings$model[i],
                        ...
                        `AR (-0.8)` = "AR-0.8",
                        `block (0.5)` = "block0.5")
    file_root <- paste0("../figs/dBH-mvgauss",
                        "-", model_tmp,
                        "-n", settings$n[i],
                        "-pi1", settings$pi1[i],
                        "-side", settings$side[i],
    ...
    plot <- data %>%
        select(-`FDR (init)`, -`power (init)`, -secBH,
               -model, -n, -pi1, -side, -alpha) %>%
        gather("type", "value", -method, -safe, -color) %>%
        ggplot(aes(x = method, y = value,
                   fill = color, color = color)) +
        geom_bar(stat = "identity", width = 0.8) +
        facet_grid(type ~ safe, scales = "free_x") +
        scale_colour_identity() +
        scale_fill_identity() +
        scale_x_discrete(labels = methods_labels) +
        scale_y_continuous(breaks = c(0.05, 0.15, 0.25, 0.35)) + 
        geom_hline(data = reference, aes(yintercept = value, color = color), linetype = "longdash") +
        ylab("") + 
        theme_bw() +
    ...
    secBH <- data %>% select(model, n, pi1, side, alpha,
                             method, secBH) %>%
        rbind(secBH, .)
}

filename <- paste0("../data/dBH_mvgauss_secBH.RData")
