...
load("../data/dBH_lm_aggregate.RData")
...
settings <- res %>%
    select(model, n, p, pi1, side, alpha) %>%
    unique
secBH <- data.frame()    
...
for (i in 1:nrow(settings)){
    ...
    if (settings$n[i] >= 2 * settings$p[i]){
        data <- data %>%
            mutate(method_type =
                       factor(method_type,
                              levels = c(0, 2, 1),
    ...
    secBH <- data %>% select(model, n, p, side, alpha,
                             method, secBH) %>%
        rbind(secBH, .)
}

filename <- paste0("../data/dBH_lm_secBH.RData")
